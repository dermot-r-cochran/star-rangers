...
- **Optional giscus discussion forum** at `/forum/` (`src/forum/index.md`), powered by [giscus](https://giscus.app/) and backed by GitHub Discussions on a repo of the clone owner's choosing. Opt-in per clone via four new `deploy.conf` keys — `GISCUS_REPO`, `GISCUS_REPO_ID`, `GISCUS_CATEGORY`, `GISCUS_CATEGORY_ID` (plus their `ALT_<id>_GISCUS_*` per-alt-domain variants) — all four required together, read into `src/_data/site.js`'s new `site.giscus` object, which gates both the widget itself and a "Forum" nav link that only appears once the forum is enabled. Every domain's forum maps to one fixed, shared discussion rather than per-page comments, and follows that clone's own `THEME` for light/dark rather than the visitor's OS preference. `src/static/.htaccess`'s CSP gains `frame-src https://giscus.app` for the widget's comment iframe. Leaving all four keys unset (the default, and always true for local dev and the GitHub Pages build) is unchanged no-forum behavior.
