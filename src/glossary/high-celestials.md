---
layout: glossary-entry.njk
title: "High Celestials"
id: high-celestials
category: "Cosmic Cascade"
short: "Legacy designation for the Telearch tier. Canonically: High Celestials = Telearchs."
related:
  - "Telearch"
  - "Celestials"
  - "Concordant"
tags: [high-celestials, telearch, cascade, legacy]
---

**High Celestials** is a legacy designation for the tier now formally known as **Telearchs**.

In all current canon, High Celestials and Telearch are synonymous. Historical records and earlier documents may continue to use "High Celestials". The correct current term is Telearch.

**Example:** Archival Survey Corps documents predating the terminology revision refer to "High Celestial oversight" of a boundary incident; the same event described in a current report would name the responsible Telearch instead.

See: [Telearch](/star-rangers/glossary/telerarch/) for the full current definition.
