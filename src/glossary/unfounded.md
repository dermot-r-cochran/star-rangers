...
**The plainest figure the record has for the class is deadwood.** Dead wood on a living tree keeps the tree's shape exactly — the branch stands where a branch would stand, and from any distance it is a branch — and nothing supplies it. It is not fought. It is shed, or pruned, and what the tree loses in the shedding is nothing that was the tree. Each clause is one of the findings above put in a gardener's words: the accurate shape, the absent membership, the structure that borrows and is not fed, the ending by starvation rather than defeat, and the observation the countermeasures rest on, that a thing supplied by nothing does not bear weight — press it with a clear question, a name, a refusal, and it goes. The figure has one edge, and the record keeps it: dead wood was once the tree's own wood, and no Unfounded thing was ever founded. The figure describes how the class *stands* to what it counterfeits, never where it came from. The gardener's other half — that the pruning is for the growth to come — is the [Limitation](/star-rangers/codex/cosmic-limitation-on-evil/)'s claim in a gardener's words, and the record registers it as it registers the Limitation: a purpose that may be claimed, and is not specified.

**What marks an Unfounded thing is counterfeit.** Each named mode wears the shape of something founded — a person, a companion animal, an object of desire, a grief that deserves comfort — without being a member of that kind. The shape is accurate. The membership is not. This is why animals detect them more reliably than instruments or people do: the counterfeit is aimed at the social registers, and a creature that declines without needing a reason is reading the membership rather than the shape.
...
above it, and that is what gives a kind its membership: something decided what
it would be. The Unfounded are underwritten by no origin condition at all, so
...
- **Two things filed under one name need have nothing else in common.** The
  name is a description of a failure, and failures resemble each other without
  being related.
- **Nothing is obliged to stay what it was filed as.** An entity presenting in
  one mode and later in another has not transformed, because there was no kind
  to depart from. The Archive treats a reclassification as a correction to the
  file rather than an event in the world.
- **The counters are matched to modes, not to kinds.** This is why the
  [countermeasures](/star-rangers/lore/predatory-entities/) look unrelated to
...
observed to be capable of, which is a real and operationally load-bearing
difference. It is a description of behaviour, not evidence of design, and the
Archive keeps it for the same reason it keeps any other measurement.
...
Devotional traditions keep their own vocabulary for these classes and are not obliged to adopt the Archive's; see the codex record for readings the Archive registers without endorsing. One such reading is registered beside the rule for *angel* in the [canonical guide](/star-rangers/lore/glossary/canonical-glossary-and-migration-guide/): the church-space tradition reads its own scripture's *demon* and *unclean spirit* as figurative or, taken literally, as this class — the whole class, whichever mode the counterfeit runs in — and yields to the record on the one point where the two part, the adversary-in-chief.
